...
public class AttendanceServiceImpl implements AttendanceService {
	...
	public Attendance saveAttendance(AttendanceDto attendanceDTO) {
		...
		YearMonth monthYear = (attendanceDTO.getMonthnyear() != null) ? attendanceDTO.getMonthnyear() : YearMonth.now();
	...
	public void markAbsent(Long studentId, LocalDate date) {
		...
		Optional<Attendance> optionalAttendance = attendanceRepository.findByStudentRegisterIdAndMonthnyear(student,
				monthYear);
	...
	public void markSundaysForSchool(Long udiseNo, YearMonth monthYear) {
		// Find the school
		School school = schoolRepository.findById(udiseNo).orElseThrow(() -> new RuntimeException("School not found"));
	...
	public void markHolidayForSchool(Long udiseNo, LocalDate date) {
		...
		if (attendances.isEmpty()) {
			throw new RuntimeException(
					"No attendance records found for school with UDISE: " + udiseNo + " for " + monthYear);
		}
	...
	public void recalculateAttendanceStatistics(Long udiseNo, YearMonth monthYear) {
		...
		for (Attendance attendance : attendances) {
			...
			double percentage = (workDays > 0) ? ((double) presentCount / workDays) * 100 : 0;
			attendance.setTotalPPercentage(percentage);
